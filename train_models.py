...
def train_models_for_pair(client: GateIOClient, pair: str, interval: str = '15m'):
    ...
    X_val, y_val = data_pipeline.create_sequences(
        val_df, feature_cols, 'target', Config.LOOKBACK_WINDOW, Config.PREDICTION_HORIZON
    )
    ...
    logger.info(f"Created {len(X_train)} training and {len(X_val)} validation sequences")
