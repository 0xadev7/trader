...
class TradingEnv:
    ...
    def reset(self) -> np.ndarray:
        ...
        self.position = 0.0  # Position size (0 = no position, 1 = max long, -1 = max short)
    ...
    def _get_state(self) -> np.ndarray:
        ...
        account_features = np.array([
            ...
            (portfolio_value - self.initial_balance) / self.initial_balance,  # Return
            len(self.equity) > 1 and (self.equity[-1] - self.equity[-2]) / self.equity[-2] or 0,  # Equity change
            current_price / self.prices[max(0, self.current_step - 20)] if self.current_step > 0 else 1.0  # Price momentum
        ])
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if action == 1:  # Buy
            if self.position < 0.95:  # Max position
                ...
                cost = trade_amount * self.balance * current_price * (1 + self.commission)
            ...
            if self.position > -0.95:  # Max short position
                ...
                if self.position > 0:  # Close long
                    revenue = trade_amount * self.balance * current_price * (1 - self.commission)
        ...
        equity_change = (portfolio_value - prev_equity) / prev_equity if prev_equity > 0 else 0
