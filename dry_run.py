...
class DryRunSimulator:
    ...
    def __init__(self, duration_days: int = 7):
        ...
        if not api_key or not api_secret:
            logger.warning(
                "API credentials not set. Dry-run needs API keys to fetch market data."
            )
            ...
            if pair in self.risk_manager.positions:
                ...
                if sl_tp:
                    self.simulate_close_position(
                        pair, current_price, timestamp, reason=sl_tp
                    )
                    return
                ...
                if signal != position["side"] and signal != "hold":
                    self.simulate_close_position(
                        pair, current_price, timestamp, reason="reverse"
                    )
            ...
            if signal in ["buy", "sell"] and confidence > 0.6:
                if pair not in self.risk_manager.positions:
                    ...
                    if validation["allowed"]:
                        ...
                        logger.info(
                            f"[DRY-RUN] {timestamp.strftime('%Y-%m-%d %H:%M:%S')} - "
    ...
    def print_statistics(self):
        ...
        for pair in list(self.risk_manager.positions.keys()):
            ...
            if last_price > 0:
                self.simulate_close_position(
                    pair, last_price, datetime.now(), reason="end_of_dry_run"
                )

        # Calculate final statistics
        duration = (datetime.now() - self.start_time).total_seconds() / 3600  # hours
        total_return = (final_equity - self.stats["start_equity"]) / self.stats[
            "start_equity"
        ]
        win_rate = (
        ...
        max_drawdown = (
            self.stats["max_equity"] - self.stats["min_equity"]
        ) / self.stats["max_equity"]
        ...
        results_path = os.path.join(
            Config.LOG_PATH,
            f"dry_run_results_{self.start_time.strftime('%Y%m%d_%H%M%S')}.json",
        )
    ...
    def start(self):
        ...
        logger.info(
            f"Running {total_iterations} iterations (every {iteration_interval_minutes} minutes)\n"
        )
