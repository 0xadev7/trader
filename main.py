...
class TradingBot:
    ...
    def __init__(self):
        ...
        if not Config.GATE_API_KEY or not Config.GATE_API_SECRET:
            raise ValueError("Gate.io API credentials not configured. Please set GATE_API_KEY and GATE_API_SECRET in .env")
    ...
    def execute_trade(self, pair: str, signal: str, current_price: float, confidence: float):
        ...
        try:
            ...
            if pair in self.risk_manager.positions:
                ...
                if signal != current_side and signal != 'hold':
                    logger.info(f"Reversing {pair} position from {current_side} to {signal}")
    ...
    def close_position(self, pair: str, current_price: float, reason: str = 'signal'):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Closed {pair} position: P&L = ${result['pnl']:.2f} ({result['pnl_pct']:.2%})")
            except Exception as e:
        ...
        try:
            ...
            for balance in balances:
                if balance['currency'] == Config.BASE_CURRENCY:
                    total_equity += float(balance.get('available', 0)) + float(balance.get('locked', 0))
    ...
    def process_pair(self, pair: str):
        ...
        try:
            ...
            logger.info(f"{pair}: Signal={signal}, Confidence={confidence:.2f}, Price=${current_price:.2f}")
