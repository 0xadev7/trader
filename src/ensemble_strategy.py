...
class EnsembleStrategy:
    ...
    def __init__(
        ...
        self.weights = {"lstm": 0.35, "transformer": 0.35, "rl": 0.30}
    ...
    def predict(self, X: np.ndarray, prices: np.ndarray) -> Dict[str, any]:
        ...
        if self.lstm_model is not None:
            try:
                ...
                confidences["lstm"] = min(
                    abs(predictions["lstm"]) / (pred_std + 1e-8), 1.0
                )
        ...
        if self.transformer_model is not None:
            try:
                ...
                confidences["transformer"] = min(
                    abs(predictions["transformer"]) / (pred_std + 1e-8), 1.0
                )
            except Exception as e:
        ...
        # Ensemble prediction
        ensemble_signal, ensemble_confidence, predicted_return = (
            self._combine_predictions(predictions, confidences)
        )
    ...
    def update_model_performance(self, model_name: str, pnl: float):
        ...
        if model_name in self.model_performance:
            ...
            if len(self.model_performance[model_name]["returns"]) > 100:
                self.model_performance[model_name]["returns"] = self.model_performance[
                    model_name
                ]["returns"][-100:]
